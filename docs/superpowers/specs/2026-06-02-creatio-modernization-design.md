...
**Scope:** Targeted modernization now, structured to be verification-ready later (B-ready). License stays BSL-1.1 by decision (see §7).
...
## 7. B-readiness & licensing (documented, not built now)
...
| File | Change |
|---|---|
| `nodes/Creatio/Creatio.node.ts` | Add Authentication dropdown + credential `displayOptions`; route HTTP through transport; modern error handling, `itemIndex`, `continueOnFail`. |
